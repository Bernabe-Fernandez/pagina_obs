export default function Procesos () {
  return (
    <section className="w-full py-16 bg-white">
      <div>

        {/* Descripción */}
        <div className="text-center max-w-7xl mx-auto mb-60 text-[20px]">
          <p className="text-[#2E6092] leading-relaxed">
            La industria agroalimentaria abarca una amplia variedad de procesos relacionados con el manejo,
            transformación y empaque de productos de origen agrícola. En este sector, el transporte interno
            debe garantizar un flujo continuo del producto sin comprometer su calidad, frescura o integridad,
            incluso en entornos donde la humedad, el polvo y los residuos orgánicos están presentes de forma constante.
          </p>
        </div>

        {/* Título */}
        <h2 className="text-3xl font-bold text-center text-azulobs-500 mb-20">
          PROCESOS COMUNES
        </h2>

        {/* Sección de 4 cartas */}
        <div className="grid grid-cols-4 w-3/4 gap-0 mx-auto">

          {/* CARTA 1 */}
          <div className="bg-white shadow-md rounded-lg overflow-hidden scale-[0.90]">
            <div className="w-full relative">
              <img
                src="/images/Industria/proceso/imagen1.jpg"
                className="w-full h-full object-cover object-bottom"
              />

              {/* Recuadro azul transparente */}
              <div className="absolute bottom-[-3px] w-full h-[80px] bg-gradient-to-b
                from-[#3b8cff]/80
                via-[#3b8cff]/40
                to-[#3b8cff]/70
                flex items-center justify-center">
                <h3 className="font-semibold text-white text-[20px] leading-tight drop-shadow-md px-6 text-center">
                  Transporte de producto a granel
                </h3>
              </div>
            </div>
          </div>

          {/* CARTA 2 */}
          <div className="bg-white shadow-md rounded-lg overflow-hidden scale-[0.90]">
            <div className="w-full relative">
              <img
                src="/images/Industria/proceso/imagen2.jpg"
                className="w-full h-full object-cover object-bottom"
              />

              <div className="absolute bottom-[-3px] w-full h-[80px] bg-gradient-to-b
                from-[#3b8cff]/80
                via-[#3b8cff]/40
                to-[#3b8cff]/70
                flex items-center justify-center">
                <h3 className="font-semibold text-white text-[20px] leading-tight drop-shadow-md px-6 text-center">
                  Transferencia entre etapas de proceso
                </h3>
              </div>
            </div>
          </div>

          {/* CARTA 3 */}
          <div className="bg-white shadow-md rounded-lg overflow-hidden scale-[0.90]">
            <div className="w-full relative">
              <img
                src="/images/Industria/proceso/imagen3.jpg"
                className="w-full h-full object-cover object-bottom"
              />

              <div className="absolute bottom-[-3px] w-full h-[80px] bg-gradient-to-b
                from-[#3b8cff]/80
                via-[#3b8cff]/40
                to-[#3b8cff]/70
                flex items-center justify-center">
                <h3 className="font-semibold text-white text-[20px] leading-tight drop-shadow-md px-6 text-center">
                  Alimentación de líneas de producción
                </h3>
              </div>
            </div>
          </div>

          {/* CARTA 4 */}
          <div className="bg-white shadow-md rounded-lg overflow-hidden scale-[0.90]">
            <div className="w-full relative">
              <img
                src="/images/Industria/proceso/imagen4.jpg"
                className="w-full h-full object-cover object-bottom"
              />

              <div className="absolute bottom-[-3px] w-full h-[80px] bg-gradient-to-b
                from-[#3b8cff]/80
                via-[#3b8cff]/40
                to-[#3b8cff]/70
                flex items-center justify-center">
                <h3 className="font-semibold text-white text-[20px] leading-tight drop-shadow-md px-6 text-center">
                  Manejo previo y posterior al empaque
                </h3>
              </div>
            </div>
          </div>

        </div>

      </div>
    </section>
  );
}

